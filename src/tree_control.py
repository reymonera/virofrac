...
# This function generatea an ANI tree based on a vClust ANI output.
# It requires the matrix generated by vClust to then generate a
# tree using ete3. Then, it uses linkage to apply a UPGMA clustering.
def get_ani_tree(fasta_file, output_dir):
    ...
    nodes = {i: Tree(name=labels[i]) for i in range(num_otus)}
